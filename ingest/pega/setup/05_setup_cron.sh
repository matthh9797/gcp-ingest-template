...
SVC_URL=$(gcloud run services describe ingest-pega-daily --format 'value(status.url)')
echo $SVC_URL
echo $SVC_EMAIL
...
gcloud scheduler jobs create http pegadaily \
       --description "Ingest Pega data using cloud run" \
